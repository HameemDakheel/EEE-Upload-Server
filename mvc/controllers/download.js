...
const dirname = "C:\\Users\\HAMIM\\Documents\\CodeProjects\\EEE-Upload-Server";
...
function downloadFiles(req, res, next) {
  if (req.method === "POST") {
    ...
    res.set("Content-Disposition", `attachment; filename=${req.path}.zip`);
      zip.pipe(res);
    for (let option of options) {
      let name = option.split("/").slice(2);
      let dirPath = path.join(dirname, name.join("/"));
      name = name[name.length - 1];
      if (!extname(name)) {
        zip.directory(dirPath, name + "/");
        console.log("here!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
      } else {
        zip.file(dirPath, { name: name });
      }
...
serveIndex.html = function _html(
  ...
  stat(path, files, function (err, stats) {
    ...
    var filesList = getFilesObj(fileList, dir);
    var locals = {
      directory: path,
      files: filesList,
      paths: getPaths(dir),
      style: iconStyle(fileList, true),
    };

    // render html
    render(locals, res, function (err, body) {
      if (err) return next(err);
    });
  });
};

function getFilesObj(files, dir) {
  var newFileList = files.map(function (file) {
    ...
    if (isDir) {
      fileIcon = { fileName: "folder.png" };
    } else {
      fileIcon = iconLookup(file.name);
    }
    // if (isDir) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if (part) {
      ...
      crumb.push({
        ...
        url: escapeHtml(parts.slice(0, i + 1).join("/")),
      });
    }
  }
...
function iconStyle(files, useIcons) {
  ...
  for (i = 0; i < files.length; i++) {
    ...
    selector = "." + icon.className;

    if (!rules[iconName]) {
      rules[iconName] = "background-image: url(data:image/png;base64,);";
      selectors[iconName] = [];
      list.push(iconName);
    }
